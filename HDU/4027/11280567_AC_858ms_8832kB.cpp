...
void init(int l,int r, int root){
    ...
    init(l,tree[root].mid,root<<1);
    init(tree[root].mid+1,r,root<<1|1);
	pushup(root);
}
void update(int l,int r,int root)
{
    ...
    if(tree[root].l==tree[root].r && tree[root].l==l &&tree[root].r==r)
    {
        ...
        if(tree[root].sum<=1) tree[root].flag=0;
        return ;
    }
    if(r<=tree[root].mid)
            update(l,r,root<<1);
...
ll query(int l,int r, int root){
    ...
    if(r<=tree[root].mid)ans+=query(l,r,root<<1);
    else if(l>tree[root].mid)ans+=query(l,r,root<<1|1);
    else
        ans+=( query(l,tree[root].mid,root<<1)+ query(tree[root].mid+1,r,root<<1|1) );
